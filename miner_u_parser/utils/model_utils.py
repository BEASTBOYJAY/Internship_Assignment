...
from miner_u_parser.utils.boxbase import get_minbox_if_overlap_by_ratio
    ...
    for i, (xmin, ymin, xmax, ymax, area, score, block) in enumerate(block_info):
        ...
        blocks_inside = [
            ...
            for j, (
                ...
                j_score,
                j_block,
            ) in enumerate(block_info)
            if i != j
            and j not in marked_indices
            and is_inside(block_info[j], block_info[i], overlap_threshold)
        ]
...
def get_res_list_from_layout_res(
    ...
    filtered_table_res_list, table_need_remove = remove_overlaps_min_blocks(
    ...
    if len(filtered_table_res_list) < len(table_res_list):
        ...
        tables_to_remove = [
            table for table in table_res_list if id(table) not in kept_tables
        ]
    ...
    blocks_to_remove = remove_overlaps_low_confidence_blocks(
        combined_res_list, overlap_threshold
    )
    # 移除需要删除的blocks
