...
class RapidTableModel(object):
    ...
    def batch_predict(self, table_res_list: List[Dict], batch_size: int = 4) -> None:
        ...
        with tqdm(
            total=len(not_none_table_res_list), desc="Table-wireless Predict"
        ) as pbar:
            for index in range(0, len(not_none_table_res_list), batch_size):
                batch_imgs = [
                    cv2.cvtColor(
                        np.asarray(not_none_table_res_list[i]["table_img"]),
                        cv2.COLOR_RGB2BGR,
                    )
                    for i in range(
                        index, min(index + batch_size, len(not_none_table_res_list))
                    )
                ]
                batch_ocrs = [
                ...
                for i, result in enumerate(results):
                    if result.pred_html:
                        not_none_table_res_list[index + i]["table_res"][
                            "html"
                        ] = result.pred_html
