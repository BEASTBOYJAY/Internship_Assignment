    ...
    def batch_predict(
        ...
        for img in imgs:
            ...
            if img_aspect_ratio > 1.2:
                ...
                normalized_h = (
                    (img_height + RESOLUTION_GROUP_STRIDE) // RESOLUTION_GROUP_STRIDE
                ) * RESOLUTION_GROUP_STRIDE  # 向上取整到RESOLUTION_GROUP_STRIDE的倍数
                normalized_w = (
                    (img_width + RESOLUTION_GROUP_STRIDE) // RESOLUTION_GROUP_STRIDE
        ...
        for group_key, group_imgs in tqdm(
            resolution_groups.items(), desc="Table-ori cls stage1 predict", disable=True
        ):
        ...
        if len(rotated_imgs) > 0:
            ...
            with tqdm(
                total=len(rotated_imgs),
                desc="Table-ori cls stage2 predict",
                disable=True,
            ) as pbar:
